          ...
          <table className="min-w-full">
            <thead className="bg-dashboardBg text-secondaryBg py-2 text-sm font-lora w-full">
              <tr className="whitespace-nowrap">
                <th className="px-4 py-2 text-sm font-lora text-left">Profile</th>
                <th className="px-4 py-2 text-sm font-lora text-left">Name</th>
                <th className="px-4 py-2 text-sm font-lora text-left">Position</th>
                <th className="px-4 py-2 text-sm font-lora text-left">Department</th>
                <th className="px-4 py-2 text-sm font-lora text-left">Created By</th>
                <th className="px-4 py-2 text-sm font-lora">Action</th>
              </tr>
            </thead>
            <tbody>
              {members.map((member: MemberProp) => (
                <tr
                  key={member.id}
                  className="whitespace-nowrap border-b border-border"
                >
                    <td className="px-4 py-3 text-xs font-lora">
                    <Image src={member.profile as string} alt={member.name} width={60} height={60} className="object-contain" />
                  </td>
                  <td className="px-4 py-3 text-xs font-lora">
                    {member.name}
                  </td>
